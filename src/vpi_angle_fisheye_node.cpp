...
mvs::PointMat3 get_xyz(double fov_x, double fov_y, const mvs::Shape_t& shape) {
    ...
    mvs::PointMat1 xx = ax.matrix().replicate(1,shape.h);
    Eigen::Matrix<Scalar_t,Eigen::Dynamic,Eigen::Dynamic> yy_square = ay.matrix().replicate(shape.w,1);
    yy_square.resize(1,N);
    // mvs::PointMat1 yy = Eigen::Map<Eigen::Matrix<Scalar_t,1,Eigen::Dynamic>>(ay.matrix().replicate(shape.w,1).transpose().data(),1,N);
    mvs::PointMat1 yy = yy_square;
    // The xyz coordinates of pixels in the pinhole camera.
    mvs::PointMat3 xyz;
    xyz.resize( 3, N );

    xyz.row(0) = xx.array().cos().matrix();
     
    
    Eigen::Matrix<Scalar_t, Eigen::Dynamic, Eigen::Dynamic> temp_row = 
    yy.array().sin().matrix();
    std::cout << "(" << temp_row.rows() << ", " << temp_row.cols() << ")"<< std::endl;

    xyz.row(0) = xyz.row(0)*(temp_row.asDiagonal()); 

    xyz.row(1) = yy.array().sin().matrix();
    temp_row =  xx.array().sin().matrix();
    xyz.row(1) = xyz.row(1)*temp_row.asDiagonal();
    ...
    for (int i = 0; i< N; i ++)
    {
        pts3d.push_back(cv::Point3f(xyz.coeff(0,i),xyz.coeff(1,i),xyz.coeff(2,i)));
    }
